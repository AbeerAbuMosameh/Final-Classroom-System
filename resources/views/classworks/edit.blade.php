...
@section('content')
    ...
    <form
        action="{{ route('classrooms.classworks.update', ['classroom' => $classwork->classroom->id, 'classwork' => $classwork->id, 'type' => $type]) }}"
        method="POST">
        @csrf
        @method('PUT')
        <div class="row">
                ...
                <div class="card card-custom border">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            <label>Title</label>
                            <input type="title" name="title" value="{{ $classwork->title }}" class="form-control"
                                   placeholder="Title"/>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="students[]" multiple>
                                @foreach ($classwork->classroom->students as $student)
                                    <option value="{{ $student->id }}"
                                            @if (isset($assigned) && in_array($student->id, $assigned ?? [])) selected @endif>
                                        {{ $student->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    @if($type == 'assignment')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="topic_id" id="topic_id">
                                ...
                                @foreach ($classwork->classroom->topics as $topic)
                                    <option value="{{ $topic->id }}" s
